...
- **Multi-source text input** — typed text, image upload, live camera capture, PDF, and DOCX, all through a single unified pipeline
- **OCR text extraction** — EasyOCR with a Tesseract fallback, supporting English and Hindi
- **Dyslexia-friendly formatting** — configurable line, letter, and word spacing with Mild / Moderate / Severe severity presets
- **AI-powered text simplification** — optional vocabulary simplification via Google Gemini, with graceful degradation when no API key is configured
- **Reading aids** — Reading Ruler and Focus Line mode to help track reading position and reduce distraction
- **Text-to-Speech with word highlighting** — synchronised audio and visual reading support
- **Dyslexia-friendly typography** — OpenDyslexic (bundled, offline) and Lexend typefaces
- **Firebase Authentication** — email/password registration and login
- **Reading library** — locally saved readings for quick access later
...
AksharAlly is under active development. Core input processing, formatting, simplification, and authentication flows are implemented and functional; accessibility refinements and additional platform support are ongoing.
...
#    Place your firebase_key.json inside backend/
...
|---|---|---|---|
| `GEMINI_API_KEY` | `backend/.env` | Optional | Enables AI text simplification via Google Gemini. Without it, `/api/simplify-text` returns the original text unchanged. |
| `firebase_key.json` | `backend/` | Optional | Enables server-side Firebase auth (`/auth/register`, `/auth/verify-token`). Without it, those routes are disabled; the rest of the API works normally. |
...
- **Dyslexia-specific typefaces** such as OpenDyslexic (with weighted bottoms on each glyph) and Lexend (designed specifically to reduce visual stress) are commonly recommended to improve reading comfort for many dyslexic readers.
...
When Text-to-Speech is active, the `HighlightedTextView` widget highlights each word as it is spoken. This synchronises the auditory and visual channels, a technique commonly associated with improved comprehension and reduced re-reading for dyslexic users.

### Colour Themes

High contrast is not always the answer — some dyslexic users find dark text on a bright white background particularly difficult. The Settings screen offers multiple colour themes so users can find the background/text combination that works best for them.
...
This project is designed, developed, and maintained as an independent accessibility project. Contributions, feedback, and collaboration are welcome — feel free to open an issue or submit a pull request.

---

## License

This repository does not yet include a license file. Until a license is added, all rights to the source code are reserved by the author. A formal license (e.g. MIT) will be added in a future update.
